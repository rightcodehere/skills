---
name: rc-skill-updater
description: >
  Syncs the local rc-* skills folder with upstream sources
  ...
  applies selected updates with the rc- prefix convention intact.
...
|---|---|
| `/rc-skill-updater check` | Scan all upstreams; report new skills and changed files |
| `/rc-skill-updater sync` | Interactive: pick which updates to apply |
| `/rc-skill-updater diff <skill-name>` | Show line-level diff for one local skill vs its upstream |
...
python skills/rc-skill-updater/scripts/check.py
```
...
python skills/rc-skill-updater/scripts/apply.py --report-path "<path from check output>"
```
...
- The `name:` field in every `SKILL.md` is **always rewritten to the local `rc-*` name** — both when syncing changed files and when adding new skills. Upstream skill names are never written to disk.
...
1. Create `skills/rc-<name>/`.
2. Download each file via the API (decode base64 with PowerShell as shown above).
3. Set `name: rc-<name>` in `SKILL.md` frontmatter.
4. **For mattpocock/skills**: update any bare skill invocation references (`diagnose` → `rc-diagnose`).
